class Solution {
    ...
    int recursion(int i,int buy,int cap,int[] prices,int [][][] dp){
        ...
        if(buy==1){
            return dp[i][buy][cap]=Math.max(-prices[i]+recursion(i+1,0,cap,prices,dp),recursion(i+1,1,cap,prices,dp));
        }
        else{
            return dp[i][buy][cap]=Math.max(prices[i]+recursion(i+1,1,cap-1,prices,dp),recursion(i+1,0,cap,prices,dp));
        }
    }
